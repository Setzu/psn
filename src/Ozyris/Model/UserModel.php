...
class UserModel extends AbstractModel
{
    ...
    /**
     * @param array $aInfosUser
     * @return bool
     * @throws \Exception
     */
    public function insertUserByInfosUser(array $aInfosUser)
    {
        $sql = "INSERT INTO users (username, email, password, admin) VALUES (:username, :email, :password, :admin)";
        $stmt = $this->bdd->prepare($sql);

        try {
            $stmt->bindParam(':username', $aInfosUser['username']);
            $stmt->bindParam(':email', $aInfosUser['email']);
            $stmt->bindParam(':password', $aInfosUser['password']);
            $stmt->bindParam(':admin', $aInfosUser['isAdmin']);
...
//                $aSqlError = $stmt->errorInfo();
     ...
     * @return array
     */
    public function selectUserByUsernameOrEmail($value)
    {
